...
fun UsageDialog(onDismiss: () -> Unit) {
    ...
    AlertDialog(
        ...
        text = {
            Column(Modifier.heightIn(max = 360.dp).verticalScroll(rememberScrollState())) {
                UsageSection("日历", "点日期看那天的事，点数字右边的月历翻月；有安排的日子底下有个小点。点「＋ 添加日程」写一条，要提醒就把开关打开，到点会弹通知。写好的日程点一下能改，长按删。")
                UsageSection("纪念日", "左边那张卡点一下就能改名字、选日期、切正数还是倒数。")
                UsageSection("闹钟", "上面是大时钟，下面是闹钟条。到点会一直响到你点通知上的「暂停」或「关闭」；点「暂停」是 5 分钟后再响一次，「关闭」就停。重复可以选每天 / 工作日 / 周末 / 自己挑星期。点一下改，长按删。")
...
5. 其他
作者：不容忆。反馈与交流的 QQ 群：1076172382（项目和教程也都发在群里，装包、用法有问题进群说就行）。用着有问题、有 bug、有想法，都可以在群里说，我尽量优化。感谢喜欢。
