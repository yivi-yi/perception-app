...
一、这是什么
这是个人做的小应用，免费分享，全本地运行，不联网上传你任何东西。日历、闹钟、纪念日这些都只存在你自己手机里，卸载就没了，重要的日子建议自己另外记一份。
...
· 勿扰权限：工具盒里的「开勿扰」要用（看规则、调音量不用这个）
不想用工具盒的话，这些权限全都可以不开。

三、关于 MCP 服务
服务开在你手机的局域网里，同一个 WiFi 下的设备理论上都能连（默认是关着的，开了才生效）。这是给 AI 用的接口，能读你的日程、通知、位置这些，也能改闹钟、调音量，所以在公共 WiFi 下建议别开，或者用完记得关。默认没有设口令，谁在同一个网里都能连，这点自己心里有数就行。

四、风险说明
本应用只在公开群里分享安装包，不在应用商店发布，自己装 APK 出问题自己负责；应用里所有数据都在本机，作者也拿不到、不承担数据丢失的责任。工具盒里那些查看与操作手机的能力，请自己想清楚再用。
