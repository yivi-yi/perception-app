    ...
    AlertDialog(
        ...
        text = {
            Column(Modifier.heightIn(max = 440.dp).verticalScroll(rememberScrollState())) {
                ...
                UsageSection("工具盒里有什么", "日程、闹钟（能读能加能改能删）、位置（实时优先，拿不到会说明是多久前的）、天气（不传城市就用当前位置）、WiFi、传感器读数、应用列表、当前前台应用、读通知（现在挂着的和最近收到的分开给）、录 3 秒环境音估分贝。还有「搜歌 + 点歌」两个：先搜歌名拿到歌名/歌手/歌曲 id，再按 id 跳本机网易云播放，走的是公开接口，不用配任何东西。")
