    ...
    AlertDialog(
        ...
        text = {
            Column(Modifier.heightIn(max = 440.dp).verticalScroll(rememberScrollState())) {
                ...
                UsageSection("工具盒里有什么", "日程、闹钟（能读能加能改能删）、位置（实时优先，拿不到会说明是多久前的）、天气、WiFi、传感器读数、应用列表、当前前台应用、读屏幕文字、读通知、录 3 秒环境音估分贝，还有一个「跳网易云点歌」。点歌要在设置 → 工具盒 → 网易云 API 里填一个网易云 API 地址；不填也能用，只是帮你打开搜索页，不会直接点开那首。")
